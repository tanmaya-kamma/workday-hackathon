...
async def get_employee_jira_list_link(employee: dict[str, Any], manager: dict[str, Any] | None = None) -> dict[str, Any]:
    """Resolve one HCM employee and optionally manager and return their KAN Jira List-view URL."""
    try:
        ...
        if not emp_email:
            ...
            if jira_user and jira_user.get("accountId"):
                ...
                if manager:
                    manager_jira = await _find_jira_user(manager, project_key)
                    if manager_jira:
                        manager_account_id = manager_jira.get("accountId")
                return {
                    "jira_available": True,
                    "jira_match": True,
                    "employee": {"name": employee.get("full_name") or employee.get("name"), "jira_account_id": account_id},
                    "jira_list_url": _employee_list_url(base_url, project_key, account_id, manager_account_id),
                }
            return {
                "jira_available": True,
                "jira_match": False,
                "employee": {"name": employee.get("full_name") or employee.get("name")},
                "jira_list_url": DEFAULT_JIRA_URL,
                "message": "Jira employee email matching failed, using project fallback url.",
            }
        ...
        return {
            ...
            "jira_list_url": _employee_list_url_by_email(base_url, project_key, emp_email, mgr_email),
        }
    ...
    try:
        ...
        if emp_email:
            for issue in normalized:
                ...
                issue_reporter = (issue.get("reporter_email") or "").strip().lower()
                
                is_assignee = (issue_assignee == emp_email.strip().lower())
                is_reporter = True
                if mgr_email:
                    is_reporter = (issue_reporter == mgr_email.strip().lower())
            ...
            jira_user = await _find_jira_user(employee, project_key)
            if jira_user and jira_user.get("accountId"):
                manager_jira_user = None
                if manager:
                    manager_jira_user = await _find_jira_user(manager, project_key)

                for issue in normalized:
                    is_assignee = issue.get("assignee_account_id") == jira_user["accountId"]
                    is_reporter = True
                    if manager_jira_user and manager_jira_user.get("accountId"):
                        is_reporter = issue.get("reporter_account_id") == manager_jira_user["accountId"]

                    if is_assignee and is_reporter:
                        assigned.append(issue)
                
                is_matched = True
                manager_account_id = manager_jira_user.get("accountId") if manager_jira_user else None
                list_url = _employee_list_url(base_url, project_key, jira_user["accountId"], manager_account_id)
