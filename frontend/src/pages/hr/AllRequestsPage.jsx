...
export function AllRequestsPage({ defaultStatus = 'all' }) {
  ...
  // Combined Multi-Filter Filtering Logic (AND)
  const filteredRequests = useMemo(() => {
    return allRequests.filter((req) => {
      ...
      if (searchQuery.trim()) {
        ...
        if (!nameMatch && !deptMatch && !reasonMatch && !idMatch && !typeMatch) {
          return false;
        }
      }
  ...
  }, [allRequests, searchQuery, statusFilter, typeFilter, departmentFilter, managerFilter]);
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `LeaveTrack_Leave_Requests_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5 space-y-4">
        ...
        {/* Secondary Filter Row / Active Filters Summary */}
        <div className="flex items-center justify-between pt-2 border-t border-[#dfe5e8]/60 text-xs">
          <div className="flex items-center gap-2 text-[#687781]">
            <span>
              Showing <strong className="text-[#0f1d27]">{sortedRequests.length}</strong> of{' '}
              <strong className="text-[#0f1d27]">{allRequests.length}</strong> requests
            </span>
          </div>
      ...
      <Card className="p-0 overflow-hidden">
        {paginatedRequests.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-sm font-bold text-[#0f1d27]">No requests matched your filter criteria</h3>
            <p className="text-xs text-[#687781] mt-1 max-w-sm mx-auto">
              Try adjusting or clearing your search queries, department selections, or status filters.
            </p>
            {hasActiveFilters && (
              <Button
                variant="outline"
                      ...
                      <div className="flex items-center gap-3">
                        {req.avatar ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-[#00646f] text-white flex items-center justify-center font-bold text-[11px]">
                            {req.initials || req.employeeName?.slice(0, 2).toUpperCase() || 'EM'}
                          </div>
                        )}
                        <div>
                          <span className="font-bold text-[#0f1d27] block">{req.employeeName}</span>
                          <span className="font-mono text-[10px] text-[#687781]">{req.id}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 text-[#3e494a] font-medium">{req.department}</td>
                    <td className="py-3.5 px-4 text-[#687781]">{req.managerName || 'Sarah Mitchell'}</td>
                    <td className="py-3.5 px-4">
                      <span className="font-semibold text-[#00646f] block">{req.leaveType}</span>
                      <span className="text-[11px] text-[#687781] font-medium">{req.durationDays} Day(s)</span>
                    </td>
                    <td className="py-3.5 px-4">
                      <span className="text-[#0f1d27] font-medium block">{req.dateDisplay}</span>
                      <span className="text-[10px] text-[#687781]">{req.startDate} to {req.endDate}</span>
                    </td>
                    <td className="py-3.5 px-4">
                    ...
                    <td className="py-3.5 px-5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          className="px-3 py-1 rounded-lg text-xs font-semibold bg-[#f0f4f7] hover:bg-[#00646f] hover:text-white transition-colors cursor-pointer text-[#00646f]"
                        >
