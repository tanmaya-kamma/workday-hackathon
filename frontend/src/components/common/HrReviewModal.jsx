...
export function HrReviewModal({ isOpen, onClose, request, initialMode = 'review' }) {
  const { hrApproveLeaveRequest, hrRejectLeaveRequest, getUserBalances } = useLeave();
  ...
  const handleApprove = () => {
    setError('');
    setLoading(true);
    const success = hrApproveLeaveRequest(request.id, comment.trim());
    setLoading(false);
    if (success) {
      onClose();
    }
  ...
  const handleReject = () => {
    if (!comment || !comment.trim()) {
      setError('Please provide a mandatory reason for HR records before rejecting.');
      return;
    }
    setError('');
    setLoading(true);
    const success = hrRejectLeaveRequest(request.id, comment.trim());
    setLoading(false);
    if (success) {
      onClose();
    }
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-[#dfe5e8] flex items-center justify-between bg-[#f8fbfb]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="hr-approval-modal-title" className="text-base font-bold text-[#0f1d27]">
                HR Leave Management & Review
              </h2>
              <p className="text-xs text-[#687781]">
                Request ID: <span className="font-mono text-[#00646f] font-semibold">{request.id}</span>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="p-6 max-h-[calc(85vh-130px)] overflow-y-auto flex flex-col gap-6">
          ...
          <div className="p-4 bg-[#f5f7f8] rounded-xl border border-[#dfe5e8] flex items-center justify-between flex-wrap gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="inline-flex items-center gap-1 text-[11px] text-[#687781]">
                    <span className="material-symbols-outlined text-[14px] text-[#687781]">supervisor_account</span>
                    Manager: {request.managerName || 'Alex Rivera'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 bg-white px-3.5 py-2 rounded-xl border border-[#dfe5e8] shadow-2xs">
              <div className="text-center px-1.5">
                <span className="block text-[10px] uppercase font-bold text-[#687781]">Annual PTO</span>
              ...
              <div className="w-px h-6 bg-[#dfe5e8]"></div>
              <div className="text-center px-1.5">
                <span className="block text-[10px] uppercase font-bold text-[#687781]">Casual</span>
                <span className="text-xs font-bold text-[#3d6fa8]">{balances.casual.remaining}d left</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-3.5 rounded-xl border border-[#dfe5e8] bg-white">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <span className="text-sm font-bold text-[#0f1d27] block">{request.leaveType}</span>
                  <span className="text-xs text-[#00646f] font-semibold">{request.durationDays} Working Day(s)</span>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-xs text-[#687781]">
                    Submitted: {request.submittedDisplay || request.submittedAt || 'N/A'}
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Existing Manager / HR Review Information */}
          {(request.reviewedBy || request.hrReviewedBy || request.cancelReason) && (
            <div className="p-4 rounded-xl bg-[#f5f7f8] border border-[#dfe5e8] space-y-2">
              ...
              {request.reviewedBy && (
                <div className="text-xs text-[#3e494a]">
                  <span className="font-semibold text-[#0f1d27]">Manager ({request.reviewedBy}):</span>{' '}
                  <span className="italic">{request.reviewReason || 'Approved'}</span>
                </div>
              )}
              {request.hrReviewedBy && (
                <div className="text-xs text-[#3e494a]">
                  <span className="font-semibold text-[#00646f]">HR ({request.hrReviewedBy}):</span>{' '}
                  <span className="italic">{request.hrReviewReason || 'Approved'}</span>
                </div>
              )}
          ...
          {/* HR Action Controls (If request is pending or can be modified) */}
          {isPending && !isCancelled && (
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-4 pt-2 border-t border-[#dfe5e8]">
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="hr-comment" className="text-xs font-semibold text-[#3e494a] flex items-center justify-between">
                  <span>
                    {decision === 'reject' ? 'Rejection Reason (Required for HR records)' : 'HR Review Note (Optional)'}
                  </span>
                  {decision === 'reject' && (
                ...
                <textarea
                  ...
                  placeholder={
                    decision === 'reject'
                      ...
                      : 'Add an optional HR note (e.g. Leave balance audited and verified against statutory allowance)...'
                  }
